        ...
        <view class="trade-panel">
          ...
          <view class="leverage-buttons">
            <view class="leverage-btn" @click="showPositionModeSelector">
              <text class="leverage-btn-text">全仓</text>
            </view>
          ...
          <template v-if="currentOrderType === 0">
            ...
            <view class="input-group">
              <view class="input-row-outer">
                ...
                <view class="input-box">
                  <text class="input-label">数量 (PIPPIN)</text>
                  <view class="input-row">
                    <input 
                      class="input-field" 
                      type="digit" 
          ...
          <view class="position-info">
            <view class="info-row">
              ...
              <view class="info-value-with-icon">
                <text class="info-value">{{ availableBalance }} USDT</text>
                <u-icon name="plus-circle-fill" size="32" color="#F0B90B" />
              </view>
            </view>
            <view class="info-row">
              <text class="info-label">可开</text>
              <text class="info-value">{{ maxPosition }} BTC</text>
            </view>
            <view class="info-row">
              <text class="info-label">保证金</text>
              <text class="info-value">{{ margin }} USDT</text>
            </view>
          </view>
          ...
          <view class="position-info bottom-info">
            <view class="info-row">
              <text class="info-label">可开</text>
              <text class="info-value">0.000 BTC</text>
            </view>
            <view class="info-row">
              <text class="info-label">保证金</text>
              <text class="info-value">0 USDT</text>
            </view>
          </view>
    ...
    <u-popup v-model="showLeveragePopup" mode="bottom" border-radius="24" :safe-area-inset-bottom="true">
      <view class="popup-container leverage-popup">
        ...
        <view class="leverage-info">
          <text class="info-text">* 当前杠杆倍数最高可开：10,000 USDT</text>
...
const currentSymbol = ref('BASUSDT')
const currentPrice = ref('0.01614')
const currentPriceUsd = ref('0.01612')
const changePercent = ref(45.67)
const fundingRate = ref('0.00500% / 03:38:20')
const showPairSelector = ref(false)
const searchKeyword = ref('')
const positionMode = ref('cross')
const leverage = ref(6)
const tradeAction = ref('open')
const orderTypes = ref(['限价单', '市价单', '限价止盈止损', '市价止盈止损'])
const currentOrderType = ref(0)
const orderType = ref('limit')
const price = ref('0.43717')
...
const tempLeverage = ref(6)
const quickLeverages = ref([1, 10, 30, 100]) // 快捷杠杆选项

// 热门交易对数据
const hotPairs = ref([
  { symbol: 'BTC', price: '109518.77', change: 1.07, icon: '🟠', iconBg: '#F7931A' },
...
const allPairs = ref([
  ...
  { symbol: 'BNB', price: '1081.4', change: -2.02, icon: '🟡', iconBg: '#F3BA2F' },
  { symbol: 'XRP', price: '2.4615', change: 3.55, icon: '⚪', iconBg: '#00AAE4' },
  { symbol: 'BAS', price: '0.01614', change: 45.67, icon: '🔵', iconBg: '#4A90E2' },
...
const sellOrders = ref([
  { price: '0.01620', amount: '207.53K' },
  { price: '0.01619', amount: '195.37K' },
  { price: '0.01618', amount: '568.01K' },
  { price: '0.01617', amount: '161.74K' },
  { price: '0.01616', amount: '186.91K' }
])

const buyOrders = ref([
  { price: '0.01611', amount: '18.98K' },
  { price: '0.01610', amount: '98.74K' },
  { price: '0.01609', amount: '159.07K' },
  { price: '0.01608', amount: '160.31K' },
  { price: '0.01607', amount: '294.68K' }
])
...
const selectPair = (pair) => {
  ...
  uni.showToast({
    title: `已切换到 ${pair.symbol}USDT 永续`,
    icon: 'none'
  })
}
</script>
