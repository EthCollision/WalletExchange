    ...
    <view class="detail-cards-container">
      <view class="detail-cards-row">
        <view class="detail-card">
          ...
          <text class="card-value">99.9</text>
          <text class="card-fiat">≈ $99.79</text>
        </view>
        <view class="detail-card">
          <text class="card-label">未实现盈亏 (USDT)</text>
          <text class="card-value">-0.01</text>
          <text class="card-fiat">≈ -$0.017579</text>
        </view>
      </view>
        ...
        <view class="position-item" v-for="(position, index) in positions" :key="index">
          <view class="position-header">
            <view class="position-title">
              ...
              <text class="position-leverage">逐仓 {{ position.leverage }}</text>
          ...
          <view class="position-detail-row-2">
            <view class="position-detail-item">
              <text class="detail-label">未实现盈亏 (USDT)</text>
              <text class="detail-value" :class="{ profit: position.pnl > 0, loss: position.pnl < 0 }">{{ position.pnl }}</text>
            </view>
            <view class="position-detail-item">
              <text class="detail-label">投资回报率</text>
              <text class="detail-value" :class="{ profit: position.roi > 0, loss: position.roi < 0 }">{{ position.roi }}</text>
            </view>
          </view>
          
          <view class="position-detail-row-3">
            <view class="position-detail-item">
              <text class="detail-label">持仓数量 (BTC)</text>
              <text class="detail-value">{{ position.amount }}</text>
            </view>
            <view class="position-detail-item">
              <text class="detail-label">保证金 (USDT)</text>
          ...
          <view class="position-detail-row-3">
            <view class="position-detail-item">
              <text class="detail-label">开仓价格 (USDT)</text>
              <text class="detail-value">{{ position.entryPrice }}</text>
            </view>
            <view class="position-detail-item">
              <text class="detail-label">标记价格 (USDT)</text>
              <text class="detail-value">{{ position.markPrice }}</text>
            </view>
            <view class="position-detail-item">
              <text class="detail-label">强平价格 (USDT)</text>
...
const allAssets = ref([
  {
    ...
    marginBalance: '100.00000000',
    transferable: '94.01386546'
  },
  {
    coin: 'USDC',
    icon: '$',
    walletBalance: '0.00144189',
    unrealizedPnl: '0.00000000',
    marginBalance: '0.00144189',
    transferable: '0.00144189'
  }
])
